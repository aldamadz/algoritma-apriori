...
export function App() {
    ...
    if (loading) {
        return _jsx("div", { className: "p-6 text-sm", children: "Memuat data analisis..." });
    }
    if (!runId) {
        return (_jsxs("div", { className: "mx-auto max-w-7xl space-y-4 p-3 sm:p-4 md:p-6", children: [_jsx(CsvImportPanel, { onImported: () => void reloadData() }), _jsx(RunAnalysisPanel, { onRan: () => void reloadData() }), _jsxs("div", { className: "rounded-md border bg-white p-4 text-sm", children: ["Belum ada analysis run. Import data lalu klik panel \"Jalankan Analisis\".", _jsx(Button, { className: "mt-3", variant: "outline", onClick: () => void reloadData(), children: "Refresh" })] })] }));
    }
    return (_jsxs("div", { className: "mx-auto max-w-7xl space-y-4 p-3 sm:p-4", children: [_jsxs("div", { children: [_jsx(CsvImportPanel, { onImported: () => void reloadData() }), _jsx("div", { className: "mt-4", children: _jsx(RunAnalysisPanel, { onRan: () => void reloadData() }) }), _jsx("div", { className: "mt-4", children: _jsx(RunsHistoryPanel, { runs: runs, selectedRunId: runId, onSelect: setRunId, onRefresh: () => void reloadData(), onDelete: (id) => void deleteRun(id) }) }), _jsx("div", { className: "mt-4", children: _jsx(CompareRunsPanel, { runs: runs }) })] }), _jsx(RulesPage, { analysisRunId: runId, departments: departments })] }));
}
